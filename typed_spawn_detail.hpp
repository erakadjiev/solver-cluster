...
template< typename Handler, typename Function, typename Creator >
struct spawn_helper
{
  spawn_helper(BOOST_ASIO_MOVE_ARG( Creator) fiber_creator) : fiber_creator(BOOST_ASIO_MOVE_CAST( Creator)(fiber_creator)){}

  void operator()()
  {
    fiber_entry_point< Handler, Function > entry_point = { data_ };
    boost::fibers::fiber fiber = fiber_creator( entry_point);
    fiber.detach();
  }

  shared_ptr< spawn_data< Handler, Function > > data_;
  Creator fiber_creator;
};
...
void spawn( boost::asio::io_service& io_service,
	...
	BOOST_ASIO_MOVE_ARG( Creator) fiber_creator)
{
    detail::spawn_helper< Handler, Function, Creator > helper(BOOST_ASIO_MOVE_CAST( Creator)(fiber_creator));
    helper.data_.reset(
...
template< typename Function, typename Creator >
	...
	BOOST_ASIO_MOVE_ARG( Creator) fiber_creator)
{
    boost::fibers::asio::spawn(
        ...
        BOOST_ASIO_MOVE_CAST( Function)( function),
		BOOST_ASIO_MOVE_CAST( Creator)(fiber_creator));
}
